...
class Pyttsx3Engine(BaseTTSEngine):
    ...
    def _load_engine(self):
        ...
        try:
            ...
            try:
                # 保存原始环境变量
                old_env['PYTHONIOENCODING'] = os.environ.get('PYTHONIOENCODING', '')
                old_env['LANG'] = os.environ.get('LANG', '')
                old_env['LC_ALL'] = os.environ.get('LC_ALL', '')
            ...
            if isinstance(e, UnicodeDecodeError):
                self.logger.warning(f"PyTTSx3编码问题（已忽略）: {e}")
                # 尝试重新初始化
                try:
                    self._engine = pyttsx3.init()
                    if self._engine:
                        self.logger.info("PyTTSx3引擎重新初始化成功")
                    else:
                        raise Exception("无法初始化PyTTSx3引擎")
                except Exception as retry_e:
                    self.logger.error(f"PyTTSx3引擎重新初始化失败: {retry_e}")
                    raise retry_e
            else:
    ...
    def _run_and_wait_with_timeout(self, timeout: int = 10):
        ...
        try:
            ...
            if worker_thread.is_alive():
                ...
                # 有异常，但如果是编码相关异常，记录警告而不是抛出
                if isinstance(exception[0], UnicodeDecodeError):
                    self.logger.warning(f"PyTTSx3编码问题（已忽略）: {exception[0]}")
                else:
                    raise exception[0]
    ...
    def synthesize_to_file(self, text: str, voice_config: VoiceConfig, output_path: str = None, 
        ...
        try:
            ...
            self.logger.info(f"开始PyTTSx3文件合成，语音: {voice_config.voice_name}")
            ...
            if not output_path:
                if output_config and hasattr(output_config, 'naming_mode'):
                    ...
                    # 使用统一的命名规则
                    output_path = os.path.join(tempfile.gettempdir(), f"pyttsx3_{voice_config.voice_name}.wav")
            ...
            with self._synthesis_lock:
                ...
                try:
                    # 使用pyttsx3的save_to_file方法保存到指定文件
                    ...
                    if not os.path.exists(output_path):
                        raise Exception("输出音频文件未生成")
                    
                    if os.path.getsize(output_path) == 0:
                        raise Exception("生成的音频文件为空")
                    
                    self.logger.info(f"PyTTSx3文件合成完成: {output_path}")
                    return output_path
                    
                except Exception as e:
                    self.logger.error(f"PyTTSx3文件合成失败: {e}")
                    raise
                    
        except Exception as e:
            self.logger.error(f"PyTTSx3文件合成异常: {e}")
            raise