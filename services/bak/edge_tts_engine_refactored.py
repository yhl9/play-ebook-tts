...
class EdgeTTSEngineRefactored(BaseTTSEngine):
    """Edge TTS引擎实现 - 重构版本"""
    ...
    def synthesize(self, text: str, voice_config: VoiceConfig) -> TTSResult:
        ...
        try:
            ...
            audio_data, srt_content = self._synthesize_with_srt(text, voice_config)
    ...
    def _synthesize_audio(self, text: str, voice_config: VoiceConfig) -> bytes:
        ...
        try:
            # 直接调用统一的合成方法，只返回音频数据
            audio_data, _ = self._synthesize_with_srt(text, voice_config)
            return audio_data
        except Exception as e:
            self.logger.error(f"Edge TTS音频合成失败: {e}")
            raise
    
    def _synthesize_with_srt(self, text: str, voice_config: VoiceConfig) -> tuple[bytes, str]:
