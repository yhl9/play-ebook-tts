...
from .base_tts_engine import BaseTTSEngine, TTSEngineType, TTSVoiceInfo, TTSQuality
...
class Pyttsx3Engine(BaseTTSEngine):
    ...
    def __init__(self, engine_id: str = "pyttsx3", engine_name: str = "PyTTSx3", 
        ...
        # 确保属性在初始化时就存在
        self.api_base = None  # PyTTSx3不需要API
    ...
    def _parse_voice_registry_entry(self, voice_token: str) -> Optional[TTSVoiceInfo]:
        ...
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, registry_path) as key:
                ...
                age = self._read_registry_value(key, "Age", "Adult")
                ...
                # 如果语言代码识别失败，尝试从语音ID中提取
                if language == "en-US" and "ZH-CN" in voice_token.upper():
                    language = "zh-CN"
                elif language == "en-US" and "EN-US" in voice_token.upper():
                    language = "en-US"
    ...
    def _infer_gender_from_name(self, voice_name: str, voice_token: str) -> str:
        ...
        try:
            ...
            # 女性语音标识
            female_indicators = [
                'zira', 'huihui', 'xiaoxiao', 'cori', 'amy', 'mary', 'sarah', 
                'lisa', 'anna', 'emma', 'female', 'woman', 'girl'
            ]
            
            # 男性语音标识
            male_indicators = [
                'david', 'mark', 'richard', 'alan', 'john', 'mike', 'tom', 
                'james', 'robert', 'male', 'man', 'boy'
            ]
    ...
    def validate_voice_config(self, voice_config: VoiceConfig) -> bool:
        ...
        if voice_config.voice_name and voice_config.voice_name not in self._voices:
            self.logger.warning(f"PyTTSx3语音 {voice_config.voice_name} 不存在")
            return False
        
        return True
    
    def get_available_voices(self) -> List[TTSVoiceInfo]:
        """获取可用语音列表 - 从Windows注册表和pyttsx3引擎获取"""
        try:
            # 如果还没有加载语音，先加载
            if not self._voices:
                self._load_voices()
