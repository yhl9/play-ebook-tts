...
提供多种TTS引擎的统一服务接口，包括：
- 多种TTS引擎支持：Edge TTS、Piper TTS、pyttsx3
- 统一接口：所有引擎使用相同的调用方式
...
try:
    ...
    if PIPER_AVAILABLE:
        print("[OK] Piper TTS 从预加载模块导入成功")
    else:
        print("[WARN] Piper TTS 从预加载模块导入失败")
except Exception as e:
    PIPER_AVAILABLE = False
    PiperVoice = None
    SynthesisConfig = None
    print(f"[WARN] Piper TTS 预加载模块导入失败: {e}")
...
from services.base_tts_engine import TTSResult
...
class TTSServiceFactory:
    ...
    def create_service(cls, engine: str) -> ITTSService:
        ...
        try:
            # 检查是否已有缓存的实例
            if clean_engine in cls._engine_instances:
                return cls._engine_instances[clean_engine]
            ...
            adapter_instance = TTSEngineAdapter(engine_instance)
            
            # 缓存实例
            cls._engine_instances[clean_engine] = adapter_instance
...
class TTSService(ITTSService):
    ...
    def synthesize_to_file(self, text: str, voice_config: VoiceConfig, output_path: str = None, output_config=None, chapter_info=None, progress_callback=None) -> str:
        """合成语音到文件（支持进度回调）"""
        try:
            if not self._current_service:
                raise AudioGenerationError("没有可用的TTS服务")
            ...
            if hasattr(self._current_service, '__class__') and self._current_service.__class__.__name__ == 'IndexTTSService':
                return self._current_service.synthesize_to_file(text, voice_config, output_path, progress_callback, output_config, chapter_info)
            else:
                # 对于其他引擎，检查是否支持progress_callback参数
                import inspect
                sig = inspect.signature(self._current_service.synthesize_to_file)
    ...
    def get_all_available_voices(self) -> List[dict]:
        ...
        try:
            # 检查缓存
            if hasattr(self, '_cached_voices') and self._cached_voices is not None:
                self.logger.debug("使用缓存的语音列表")
                return self._cached_voices
            
            all_voices = []
            
            # 获取所有引擎，不仅仅是"available"状态的引擎
            all_engines = ['piper_tts', 'pyttsx3']
            ...
            # 缓存结果
            self._cached_voices = all_voices
    ...
    def synthesize_text(self, text: str, voice_config: VoiceConfig, output_path: str = None, output_config=None, chapter_info=None, progress_callback=None) -> str:
        """合成文本为音频文件（兼容旧接口，支持进度回调）"""
        return self.synthesize_to_file(text, voice_config, output_path, output_config, chapter_info, progress_callback)
